...
int main(int argc, char *argv[])
{
    ...
    if(argc >= 2)
    {
        ...
        if(param == "PlotFull")
        {
            vector<Airport> airports = ImportAirports(airportData);
            vector<Route> routes = ImportRoutes(routeData);

            PngMap pngMap(airports);
            pngMap.createMap(routes, argv[2]);
        }
        ...
        if(param == "PlotN")
        {
            ...
            for(int i = 0; i < n; ++i)
            {
                subAirports.push_back(airports[i]);
